    ...
    do q=1,ntspecies
      ...
      do r=1,ntreesold(q)
        do s=1,int(nsub)-1         
          ...
          do while (newx.ge.dataleft.and.newx.le.dataright.and.newy.ge.databottom.and. &
            newy.le.datatop.or.newx.gt.nx*dx.or.newx.lt.0.or.newy.gt.ny*dy.or.newy.lt.0)
        ...
        do s=1,ntrees(r)
          if (q.ne.r.or.t.ne.s) then
            do while(abs(tlocation(q,t,1)-tlocation(r,s,1)).lt.0.1.and.abs(tlocation(q,t,2)-tlocation(r,s,2)).lt.0.1)
              ...
              call random_number(rnumy)
              newy = rnumy*ny*dy
              do while (newx.ge.dataleft.and.newx.le.dataright.and.newy.ge.databottom.and.newy.le.datatop.or. &
                  newx.gt.nx*dx.or.newx.lt.0.or.newy.gt.ny*dy.or.newy.lt.0)
                call random_number(rnumx)
                newx = rnumx*nx*dx
                call random_number(rnumy)
                newy = rnumy*ny*dy
    ...
    do q=1,ntspecies
      ...
      do r=1,rounddown(q)
        do s=1,floor(nsub)-1
          ...
          newx = tlocation(q,r,1)
          newy = tlocation(q,r,2)
          do while (newx.ge.dataleft.and.newx.le.dataright.and.newy.ge.databottom.and.newy.le.datatop &
              .or.newx.gt.nx*dx.or.newx.lt.0.or.newy.gt.ny*dy.or.newy.lt.0)
            call random_number(rnumx)
            newx = rnumx*nx*dx
            call random_number(rnumy)
            newy = rnumy*ny*dy
